...
def pca_transformation(cfg:Box, data:pd.DataFrame) -> pd.DataFrame:
    ...
    df_pca = pd.DataFrame(data=X_pca, columns=[f'PC{i+1}' for i in range(cfg.data.pca_transformation.n_components)])
...
class Processesor():
    ...
    def split_feature_and_target(self, data:pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
